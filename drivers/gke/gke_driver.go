...
func (d *Driver) GetDriverCreateOptions(ctx context.Context) (*types.DriverFlags, error) {
	...
	driverFlag.Options["master-auth-username"] = &types.Flag{
		Type:     types.StringType,
		Usage:    "The username to use for HTTP basic authentication to the master endpoint",
		Password: true,
	}
	driverFlag.Options["master-auth-password"] = &types.Flag{
		Type:     types.StringType,
		Usage:    "The password to use for HTTP basic authentication to the master endpoint",
		Password: true,
	}
...
func (d *Driver) Update(ctx context.Context, info *types.ClusterInfo, opts *types.DriverOptions) (*types.ClusterInfo, error) {
	...
	if newState.NodePool.Autoscaling.Enabled {
		if newState.NodePool.Autoscaling.MinNodeCount < 1 || newState.NodePool.Autoscaling.MaxNodeCount < newState.NodePool.Autoscaling.MinNodeCount {
			return nil, fmt.Errorf("minNodeCount in the NodePool must be >= 1 and <= maxNodeCount")
		}
